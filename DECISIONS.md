...
- Monolithic forward implementation boundary: start with a separate `pcketlm_forward.dll` that owns session state and exposes prefill/decode/verify/commit/rollback through one ctypes boundary. The first implementation deliberately validates ABI, lifecycle, kill switch, and C-owned tentative/committed state on deterministic native tests before touching real model routing. Attempting to wire real Qwen/Qwen3/Mixtral immediately would require a stable C-side weight pointer ABI; calling back into Python tensor loaders per layer would recreate the overhead this phase is meant to remove.
