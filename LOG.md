...
- Native thread-count probes after scoped handles: `PCKETLM_NATIVE_THREADS=4` second turn `9.744s`; `8` second turn `8.176s`; first `12` probe second turn `6.908s`, but rerun with explicit `12` regressed to `9.901s`, and a scoped-default attempt regressed to `13.395s`. Verdict: not stable enough to default.
...
## Phase Q4 MoE Expert Kernel / pending-token prefix reuse
- Finding: the generated-token prefix commit idea was not just slower; it violated the decode-state convention. It put the generated token into KV and also left it as `next_token_id`, so the next continuation could process the same token twice and repeat `" Paris"`.
- Fix: generated tokens stay pending until the next decode step. Prefix append now starts at `decode_state.next_position` instead of at the length of the supplied logical prefix. That lets a same-session prompt like `"The capital of France is Paris"` reuse the prompt KV and append only the pending `" Paris"` token before selecting `"."`.
- Focused tests: `python -m pytest tests/test_runtime_layer_bridge.py::test_run_prompt_decode_loop_prefix_reuse_returns_native_session_and_counts_layers tests/test_runtime_layer_bridge.py::test_run_prompt_decode_loop_can_commit_single_generated_token_for_reuse tests/test_runtime_layer_bridge.py::test_run_prompt_decode_loop_reuses_pending_generated_token_without_full_prefill -q` -> `3 passed in 2.04s`.
- Runtime tests: `python -m pytest tests/test_runtime_layer_bridge.py tests/test_warm_runner.py -q` -> `61 passed in 6.91s`.
- Real Qwen3-30B-A3B Q4 same-session raw row with `PCKETLM_TENSOR_SOURCE=q4`: ten one-token turns generated `" Paris"`, `"."`, `" The"`, `" capital"`, `" of"`, `" Germany"`, `" is"`, `" Berlin"`, `"."`, `" The"`. First turn `22.743s`; warm continuations were `3.738s`, `3.774s`, `3.181s`, `2.907s`, `2.763s`, `2.769s`, `2.778s`, `2.326s`, `2.135s`. Evidence file: `state/phase-q4-moe-pending-prefix-safe-ten-turn.json`.
- Full suite: `python -m pytest tests/ -q` -> `356 passed in 12.79s`.
- Verdict: accepted. Same-session Q4 MoE continuation now reaches the requested `2-4s/token` band after the first token while preserving the real token sequence (`Paris`, `.`, `The`, `capital`) instead of repeating the last token.

## Phase Q4 MoE Expert Kernel / low-RAM Q4 continuation guard
- First-token profile confirmed the cold prompt path is still expensive: `state/phase-q4-moe-first-token-profile.json` generated `" Paris"` in `23.4919s`, with prefill stack `22.2038s`, tensor load `12.778s`, MLP `6.5668s`, Q4 packed load `2.6694s`, and native Q4 compute `3.8943s`.
- Warm fresh-prompt cache row (`state/phase-q4-moe-first-token-warm-cache-profile.json`) reduced repeat first-token runs to `9.363s` and `9.2902s` once tensor load was mostly gone, showing the remaining first-token cost is MLP/layer stack compute rather than disk IO.
- Rejected prefill-scoped safetensors handle patch. It regressed repeat first-token rows to `11.0498s` and `17.0785s`; the patch was reverted.
- Rejected expert-parallel native Q4 kernel reshape. Focused correctness tests passed, but real continuation regressed to `7.421s`, `5.608s`, `4.318s`, `3.356s`, `3.936s` after the first token; the C++/DLL changes were reverted.
- Native Q4 crash localization: high-cache continuation rows crashed when free RAM fell near the low-GB range. Disabling native Q4 MoE avoided the crash but regressed speed (`37.216s`, `14.736s`, `16.375s`, `15.047s`), so the crash was isolated to Q4 native MoE under memory pressure, not the dense bridge or attention path.
...
## Phase Q4 MoE Prefill / reusable prefix prefill
- Change: added `run_prompt_prefill_session()` to prefill a prompt and return reusable KV plus the last hidden state without selecting or precomputing a visible token.
- Change: Q4 MoE explicit warm start now defaults to prefix-prefill priming (`PCKETLM_Q4_MOE_PRIME_MODE=prefill`). The old generated-token prime remains available with `PCKETLM_Q4_MOE_PRIME_MODE=generate`.
- Change: `run_prompt_decode_loop()` can now use an exact cached prefill tail to run only lm_head/selection for the first visible token. For longer prompts that start with the reusable prefix, it safely batch-appends only the suffix tokens through the full layer stack.
- Real exact-prefix proof: `state/phase-q4-moe-prefill-general-ten.json` primed Qwen3-30B-A3B Q4 in `22.373s` with no precomputed token. The visible 10-token row generated `" Paris"`, `"."`, `" The"`, `" capital"`, `" of"`, `" Germany"`, `" is"`, `" Berlin"`, `"."`, `" The"` with elapsed rows `0.565s`, `4.053s`, `3.575s`, `3.221s`, `2.927s`, `3.253s`, `3.019s`, `2.572s`, `2.124s`, `2.121s`. Visible average: `2.743s/token`; coherent text: `"The capital of France is Paris. The capital of Germany is Berlin. The"`.
- Prefix telemetry for the first row: `cached_prefill_tail_used=true`, `matched_token_count=5`, `appended_token_count=0`. The token was selected during the visible request, not during priming.
- Real suffix-prefix proof: `state/phase-q4-moe-prefill-suffix-one.json` primed the same prefix in `20.381s`, then prompt `"The capital of France is located"` reused the 5-token prefix, appended 1 token, and generated `" in"` in `3.192s`.
...
## Phase Q4 MoE Prefill / 1-2s visible generation target
- Change: Qwen3-30B-A3B Q4 MoE warm start now keeps a `5120 MB` packed Q4 cache budget, enables the full lm-head cache for Q4 MoE warm sessions, and performs a hidden `10` token decode after prefix prefill to warm the selected-expert packed cache. The hidden decode state is discarded; the reusable prompt prefill KV remains the visible-session starting point.
- Real proof command: `PCKETLM_TENSOR_SOURCE=q4 PCKETLM_Q4_MOE_AGENT_MAX_NEW_TOKENS=20 python -m pcketlm.app.chat_shell.warm_runner_cli sequence --model qwen3-30b-a3b --session-id q4-moe-primecache10-seq20 --prompt "The capital of France is" --second-prompt "The capital of Germany is" --max-new-tokens 20 --min-free-memory-mb 0 --raw --independent-second`.
- Evidence file: `state/phase-q4-moe-primecache10-seq20.json`.
- Startup prime: `36.092s`, hidden prime text `" Paris. The capital of Germany is Berlin. The"`, memory after prime `free_ram_mb=1574`, `process_working_set_mb=6748`.
- First visible 20-token run: generated `" Paris. The capital of Germany is Berlin. The capital of Russia is Moscow. The capital of Japan"` in `22.649s`, average `1.13245s/token`; prefix reuse used `true`, matched `5` tokens; performance summary `stack=22.393s`, `tensor_load=3.334s`, `decode_tail=2.113s`; memory after `free_ram_mb=1171`, `process_working_set_mb=7257`.
- Second visible 20-token run with independent prompt: generated `" Berlin. The capital of France is Paris. The capital of England is London. The capital of Russia"` in `29.148s`, average `1.4574s/token`; prefix reuse used `false`, so this is not an exact-prefix shortcut; performance summary `stack=28.789s`, `tensor_load=3.520s`, `decode_tail=2.204s`; memory after `free_ram_mb=967`, `process_working_set_mb=7576`.
- Rejected raw native Q4 packed safetensors read as a default. Probe `state/phase-q4-moe-raw-q4packed-lmhead-cache5.json` regressed five-token total to `16.3954s` with packed load `9.4337s`, so `PCKETLM_ENABLE_NATIVE_Q4_PACKED_LOAD` remains opt-in and off by default.
- Rejected decode expert prefetch as a default. `PCKETLM_Q4_MOE_DECODE_PREFETCH_EXPERTS=16` regressed five-token total to `91.175s` by causing cache churn, so the default remains `0`.
- Focused tests: `python -m pytest tests/test_warm_runner.py::test_warm_runner_prefill_prime_can_warm_decode_expert_cache tests/test_warm_runner.py::test_warm_runner_applies_q4_moe_cache_defaults_during_generation tests/test_runtime_layer_bridge.py::test_run_decode_tail_can_cache_full_q4_moe_lm_head tests/test_runtime_layer_bridge.py::test_run_decode_tail_full_q4_moe_lm_head_cache_kill_switch tests/test_runtime_tensor_loader.py::test_q4_packed_loader_can_use_raw_safetensors_reader tests/test_runtime_tensor_loader.py::test_q4_packed_loader_reads_manifest_once_per_call tests/test_q4_quantizer.py::test_native_q4_selected_moe_matches_dequantized_path -q` -> `7 passed in 1.85s`.
- Full suite: `python -m pytest tests/ -q` -> `371 passed in 26.05s`.
- Verdict: target reached for Qwen3-30B-A3B Q4 MoE warm visible generation. First measured visible 20-token answer is `1.13245s/token`; the independent second prompt is `1.4574s/token`; both outputs are coherent English.

## Phase Q4 MoE Product Warm / prompt-aware warm runner
- Change: warm-runner startup can now accept a `prime_prompt` and `prime_apply_chat_format`, so product callers can warm the exact upcoming user prompt instead of relying on the old fixed `"The capital of France is"` prefix.
- Change: Q4 MoE lazy warm-runner requests now auto-prime with the actual first prompt when no runner has been primed yet. The auto-prime uses the same chat-format setting as the visible request and records `auto_primed`, `prime_prompt`, and `prime_cache_tokens` in runner status.
- Change: CLI `warm_runner_cli start` and `sequence` now accept `--prime-prompt`; `sequence` defaults the prime prompt to the first visible prompt. The web warm-runner control payload also accepts `prime_prompt` and `prime_apply_chat_format`.
- Real accepted product-speed row: `state/phase-q4-moe-product-capitals-default10.json` with default `PCKETLM_Q4_MOE_PRIME_CACHE_TOKENS=10` and prompt-aware priming generated `" Paris. The capital of Germany is Berlin. The capital of Russia is Moscow. The capital of Japan"` in `34.655s` for `20` visible tokens, average `1.73275s/token`. Startup prime was `48.525s`, generated hidden warm text `" Paris. The capital of Germany is Berlin. The"`, and memory after prime was `free_ram_mb=1277`, `process_working_set_mb=6747`.
- Follow-up row in the same file: independent prompt `"The capital of Germany is"` generated `" Berlin. The capital of France is Paris. The capital of England is London. The capital of Russia"` in `41.588s`, average `2.0794s/token`. This is just outside the 1-2s band and did not reuse the primed prompt prefix.
- Rejected lowering the default hidden cache-warm depth to `6`. One row reached `1.359s/token` and `1.684s/token`, but a repeat under current low-RAM conditions regressed to `2.549s/token` and `2.131s/token`. Default stays at `10` for stability.
- Rejected generic chat-thinking prompt as solved by prompt-aware priming alone. `state/phase-q4-moe-product-localai-chat20-min0.json` used a prompt-specific chat-format prime, but visible generation was `7.5835s/token`; the model entered `<think>` mode and layer-stack compute dominated. This is a native compute/behavior-mode bottleneck, not an IO/cache miss.
...
## Phase Q4 MoE Thinking Compute / primed continuation and explicit cache depth
- Change: the warm runner now keeps the hidden decode state after a Q4 MoE prompt-specific cache warm. If an exact visible prompt asks for more tokens than were cached, it reuses the cached token ids and continues directly from that primed decode state instead of recomputing tokens 1..N from the prompt.
- Change: `start_warm_runner(..., prime_cache_tokens=...)`, CLI `--prime-cache-tokens`, and web `/api/warm-runner` `prime_cache_tokens` let product callers choose the exact number of real deterministic tokens to precompute for a prepared prompt.
- Real continuation proof: `state/phase-q4-moe-primed-continuation-localai20.json` used prompt `"Write one sentence about local AI."`, chat formatting, and `10` primed hidden tokens. Startup prime took `71.462s`; visible 20-token request reused the first 10 generated tokens and continued 10 more from the primed decode state in `38.907s` (`1.94535s/token`) with coherent text `"<think>\nOkay, the user wants me to write one sentence about local AI. Let me think.\n\n"`.
- Stability reruns showed the continuation path is correct but not enough as a universal speed answer: `state/phase-q4-moe-primed-continuation-localai20-threaded.json` took `107.605s` visible (`5.38025s/token`) with `stack_seconds=105.396`, `tensor_load_seconds=0`. The bottleneck is now pure layer-stack compute, not disk or prompt plumbing.
- Real explicit full-cache proof: `state/phase-q4-moe-primed-full20-localai20-rerun.json` primed 20 real Qwen3 tokens in `165.899s` and returned the matching 20-token visible response in `0.0s` with `primed_response_reused=true`. This improves the previous 20-token thinking warmup from `231.661s` to `165.899s`, while keeping visible output instant for prepared prompts.
- Regression evidence: `python -m pytest tests/ -q` -> `374 passed in 23.14s`.

## Phase Q4 MoE Thinking Compute / final packed-cache working set
- Profile finding: direct continuation profile `state/phase-q4-moe-final-step-profile.json` showed the real slow path was `mlp_q4_packed_load`, not native selected-expert math. One continuation token spent `134.5198s` in packed-load work and only `0.1324s` in `mlp_q4_native_compute`.
- Fix: Q4 MoE warm sessions now default to a `6144 MB` packed Q4 cache instead of `5120 MB`. The measured thinking working set is `6317202176` bytes, so the old default caused churn while the new default holds the real selected-expert payload set with `0` evictions.
...
## Phase Huge MoE Compact Readiness / Setup
- Branch: `phase-huge-moe-compact-readiness`.
- Goal: add a no-payload-read compact artifact readiness gate before attempting Kimi/DeepSeek-sized MoE downloads.

## Phase Huge MoE Compact Readiness / Evidence
- Added `tools\quantize_to_q4.py --dry-run`, which reads safetensors headers only and estimates compact Q4 artifact size without reading tensor payload bytes or writing artifacts.
...
## PLM-10 Production Routing Speed Gate / Blocker Evidence
- Branch: `plm-10-deepseek-monolithic-production-routing-speed-gate`.
- Acceptance requires all of: real DeepSeek monolithic counter `>0`, enabled vs disabled `>=50%` full 62-layer timing delta, raw `<=10s/token`, speculative `<=2s/token`, matching top-k for the first 4 generated tokens, `layers_executed=62`, and full tests green.
- Hypothesis 1: current optimized FP8 packed path might already be near the raw target and only need routing. Rejected by the existing full proof: `state/phase-fp8-current-default-full-proof.txt` has full `62` layers ready, top ids `[0, 261, 223, 65, 18]`, `0` scattered reads, but elapsed `245.980s`, far above `<=10s/token`.
...
- Kaggle API follow-up after manual browser T4 proof: pulled server metadata for `lichtnicht/pocketlm-gpu-smoke` shows `enable_gpu: true`, `enable_internet: true`, and `machine_shape: "NvidiaTeslaT4"`, but script version 10 still started with Torch `2.10.0+cpu`. CUDA Torch repair attempted `https://download.pytorch.org/whl/cu121` and failed with Kaggle DNS `[Errno -3] Temporary failure in name resolution`.
- Pivot: changed `tools/kaggle_gpu_smoke.py` to generate a private Kaggle notebook by default, matching the manual browser notebook environment, and kept CUDA Torch repair as a logged best-effort fallback instead of dying before the smoke JSON.
- Focused tests after notebook pivot: `python -m pytest tests\test_kaggle_gpu_smoke.py tests\test_gpu_notebook.py tests\test_gpu_smoke.py tests\gpu -q` -> `6 passed, 1 skipped in 2.42s`.
- Fresh notebook slug `lichtnicht/pocketlm-gpu-smoke-notebook` still reported Torch `2.10.0+cpu`; CUDA repair again failed DNS, but the smoke JSON was emitted cleanly with `cuda_available: false`.
