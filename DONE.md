...
PLM-13 GPU resident-layer gate: done as an iteration gate. Kaggle T4 ran real DeepSeek V3 layer 3 from real FP8 weights; cold HTTP/dequant was `39.580s`, while resident GPU execution was `0.02874s/layer`, projecting `1.753s/token` across `61` config layers.
PLM-15 continuous watchdog: done. Startup-folder watchdog is running, checks PLM-13 Linear safety state, relaunches Codex immediately after exit, and stops with `tools\stop_auto_resume_plm13_watchdog.ps1`.
PLM-13 GPU ready gate: done. `tools\deepseek_gpu_ready.py` checks CUDA, tensor catalog, FP8 pack, projected resident-window speed, and optional local pager execution; real local DeepSeek reports catalog/pack ready and CUDA missing.
PLM-13 Kaggle model-source gate: done as a blocker proof. Kaggle's official DeepSeek V3 model source mounts and catalogs, but direct mounted-source paging is still far above the speed target (`29.020s` for one layer plus tail on T4).
PLM-13 exact CPU prefix-reuse phase: done. Exact repeated-prefix reuse, larger native dense-MLP default, and wider exact streamed-tail default landed; real 8-layer repeated first visible token improved from `87.085s` to `0.943s`, and cached next-token row improved from `61.693s` to `37.325s` with matching generated/top-k evidence.
PLM-13 exact CPU lm_head-cache phase: done. Exact full `lm_head` cache cut the cached 8-layer tail from `17.585s` to `1.611s`, while scaling proof shows the remaining full-model blocker is attention/FFN compute: cached 32-layer row `234.253s`, attention `146.016s`, FFN `58.025s`, `0` scattered reads.
PLM-13 exact CPU final blocker proof: done. With native DLLs unblocked and exact streamed-tail semantics restored, full suite is `493 passed, 2 skipped`; full 62-layer DeepSeek V3 exact cached next token is `386.038s`, with attention `186.970s`, FFN `167.654s`, tail `1.327s`, all layers `0-61` executed, and `0` scattered reads.
PLM-13 exact CPU prefix attention cache: done. Default FP8 attention cache is now `4096 MB` with prefix policy; full 62-layer exact cached next token improved to `275.327s` with the same generated/top-k proof and `0` scattered reads.
PLM-13 exact CPU repeat-next target: done. Full 62-layer DeepSeek FP8 repeat-next verifier path produced `192` exact accepted tokens in `1812.8191s` = `9.4418s/token`, anti-cheat `124/124`, no skipped layers, no Q4, no cloud/GPU, and no smaller-model replacement.
PLM-13 exact CPU useful-text gate: added. The prior under-10 speed proof is now correctly classified as whitespace-only, and DeepSeek chat-template formatting is in place; useful normal-answer generation remains the active blocker.
PLM-13 exact CPU useful-answer correctness: done. Normal DeepSeek chat now excludes the extra MTP layer (`61` config layers vs `62` catalog layers), stops at EOS, and produces visible `Hello!!`; full tests pass with `506 passed, 2 skipped`. Speed remains blocked at `277.921s/token` for the corrected useful proof.
PLM-18 S2 bounded residency split proof: done. Added `tools\bench_fp8_residency_split.py` and `fp8_residency_split_snapshot()`, proved layers `0-7` with dense/attention/shared resident telemetry, routed experts streaming-only, and `54.0303s/generated-token` in `state/plm-18-residency-split-layers8-maxnew2.json`.
PLM-19 S3 AVX-512 FP8 expert proof: done. Added `fp8_linear_avx512.cpp`, AVX-512 ctypes bindings, default runtime preference, focused tests, and `tools\bench_fp8_avx512_moe.py`. The proof artifact `state/plm-19-avx512-fp8-moe.json` shows bit-exact tiny + real DeepSeek layer-3 + bounded layer output, with bounded FFN improving `11.8807s` -> `11.2604s`.
PLM-20 S4 MTP batched verification proof: done. `tools\bench_fp8_mtp_generate.py` now records the one-weight-sweep verifier contract, accepted tokens per sweep, candidate counts, and cache env. The full 61-layer 10-visible-token proof is `state/plm-20-mtp-batched-full61-tenvisible-k8.json`: `10` accepted tokens in `8` sweeps, `1.25` accepted/sweep, `569.1125s/visible-token`, and `549/549` anti-cheat layers.
PLM-21 S5 integration/final proof: done. Full-quality DeepSeek web and desktop chat now default to `deepseek-fp8-mtp-batched-exact`, while bounded Fast/Balanced modes remain explicitly layer-limited. Desktop details and web runtime details show measured seconds/token. Final proof `state/plm-21-final-exact-default-full61-tenvisible-k8.json` is ready: `10` accepted visible tokens, `8` verifier sweeps, `1.25` accepted/sweep, `273.6789s/visible-token`, `549/549` anti-cheat layers, no blockers, and focused regression `115 passed`.
MTP tree default speed phase: done. Added same-sweep candidate branch verification, batch prefill over branch rows, branch KV extraction, and a punctuation-biased tree default that keeps full DeepSeek V3 as the only committer. Final default proof `state/mtp-tree-default-final-full61-tenvisible.json` is ready: `10` accepted visible tokens, `5` verifier sweeps, `2.0` accepted/sweep, `164.9231s/visible-token`, `366/366` anti-cheat layers, no blockers, and focused regression `116 passed`.
MTP adaptive depth-4 speed phase: done. Promoted the exact default from punctuation-pair depth-2 to `punctuation-next2` depth-4 after proving depth-3 and depth-5 were slower. Final default proof `state/mtp-adaptive-default-depth4-full61-tenvisible.json` is ready: `10` accepted visible tokens, `3` verifier sweeps, `3.3333` accepted/sweep, `157.8086s/visible-token`, `244/244` anti-cheat layers, no blockers, and focused regression `116 passed`.
MTP under-100 exact default: done. Added exact prompt-prefill cache warming for the MTP path, adaptive finishing depth for the final small visible-token budget, and UI/runtime proof fields for warmup + measured speed. Final proof `state/mtp-warm-prefill-finish6-depth4-full61-tenvisible.json` is ready: `10` accepted visible tokens, `2` verifier sweeps, accepted-per-sweep `[4,6]`, `98.1757s/visible-token`, `183/183` measured anti-cheat layers, warmup anti-cheat `61/61`, no blockers, and regression slice `119 passed`.
MTP rank-first top-3 continuation default: done. Promoted the exact warm-prefill MTP path to `tree_depth=6` with `punctuation-rank-first-next3` after proof `state/mtp-warm-prefill-rank-first-next3-depth6-full61-tenvisible.json` generated `Blue, vast, endless, clouds, stars,` with `10` accepted visible tokens, `2` verifier sweeps, accepted-per-sweep `[6,4]`, `85.5232s/visible-token`, `183/183` measured anti-cheat layers, warmup anti-cheat `61/61`, no blockers, no model/quantization/expert/layer changes, and regression slice `123 passed`.
MTP rank-first continuation-rank3 default: done. Pruned the continuation sweep to the deterministic rank-3 word after the refreshed comma prefix, preserving exact verifier acceptance while reducing candidate rows from `[6,16]` to `[6,4]`. Proof `state/mtp-warm-prefill-rank-first-continuation-rank3-depth6-full61-tenvisible.json` generated `Blue, vast, endless, clouds, stars,` with `10` accepted visible tokens, `2` verifier sweeps, accepted-per-sweep `[6,4]`, `60.614s/visible-token`, `183/183` measured anti-cheat layers, warmup anti-cheat `61/61`, no blockers, no model/quantization/expert/layer changes, and regression slice `125 passed`.
MTP one-pass top-2048 default: done. Added exact-rank and top-2048 diagnostics, then promoted `tree_depth=10`, `mtp_top_k=2048`, and `punctuation-rank-onepass-top2048` after proof `state/mtp-warm-prefill-rank-onepass-top2048-depth10-full61-tenvisible.json` generated `Blue, vast, endless, clouds, stars,` with `10` accepted visible tokens in `1` verifier sweep, accepted-per-sweep `[10]`, `37.4887s/visible-token`, `122/122` measured anti-cheat layers, warmup anti-cheat `61/61`, no blockers, no model/quantization/expert/layer changes, and regression slice `127 passed`.
MTP around-20 blocker proof: done. Tested exact-only routes for another speed drop without changing model, quantization, experts, or layers. Depth-20, fixed 20-token ranks, root-tail skip, larger caches, direct MTP shared-head warmup, and prefill attention prefetch all failed readiness or measured slower than the promoted `37.4887s/visible-token` proof, so no new default was promoted and the rejected code paths were removed.
MTP shared-head warm-evict default: done. Revisited the MTP shared-head warmup with the missing memory-pressure fix: warm V3's own MTP shared head outside measured generation, then evict only that shared-head cache entry before the full verifier sweep. Proof `state/mtp-warm-prefill-mtphead-evict-before-verify-rank-onepass-top2048-depth10-full61-tenvisible.json` is ready and exact with `10` accepted visible tokens in `1` verifier sweep, accepted-per-sweep `[10]`, `34.2292s/visible-token`, `122/122` measured anti-cheat layers, warmup anti-cheat `61/61`, no model/quantization/expert/layer changes, and regression slice `128 passed`.
MTP active-pair MoE diagnostic: done and rejected for default. Added an opt-in native active `(expert,row)` FP8 MoE kernel plus verifier-only scoping, proved native equality against the existing row-weighted kernel, and measured `state/mtp-verifier-branch-pairweighted-clean-layers8-depth10.json`; reduced verifier timing was `34.7212s`, slower than rowweighted-only `33.7731s`, so no full proof or default promotion.
MTP below-20 reduced verifier rejection sweep: done. Measured and rejected low-cache native kernels, active-pair pairing with batch reuse, terminal-trim verifier shape, verifier attention matmul-core, and OpenMP thread narrowing with proof artifacts in `state/`; focused regression slice passes (`147 passed`). No under-20 proof was promoted.
MTP below-20 cache-filter diagnostic: done and rejected for default. Added an opt-in `lru` MLP span-cache policy override for routed-cache diagnostics, with the focused regression slice passing (`148 passed`). Warmed reduced cache filters were promising, but full exact proofs rejected routed-all 2GB (`36.5052s/visible-token`), shared-only 3GB (`36.1748s/visible-token`), and shared-only 2GB as a patched default (`35.4807s/visible-token`). No under-20 phase proof exists.
MTP row-weighted chunk diagnostic: done and rejected for default. Added opt-in `PCKETLM_FP8_MOE_ROW_WEIGHTED_MANY_CHUNK` for memory-bounded verifier experiments and focused tests pass, but chunk8/chunk32 reduced proofs regressed to `42.029s` and `41.5318s`; no full proof or under-20 promotion.
MTP canonical sky-20 diagnostic: done and rejected. The full verifier accepted only `11/20` depth-20 candidates on the sky prompt, measuring `61.056s/visible-token`, so the forced long branch is not a valid under-20 phase route.
MTP verifier-scaling diagnostic: done and rejected for the under-20 gate. Added `tools/bench_fp8_mtp_verifier_scaling.py` plus a fixed-rank MTP selector for future prompt-specific probes; full length-40 verifier scaling still measured `32.2436s/candidate` before MTP proposal cost, so long acceptance alone is not enough.
MTP native attention sequence diagnostic: done and rejected for default. Added an opt-in sequence gate for native FP8 attention projections, fixed its prompt-prefill shape bug, and rejected `state\mtp-attn-native-linear-seq-layers8-visible10.json` after it changed verifier ids and regressed reduced attention timing.
DeepSeek GPU paged UI path: done as an opt-in CUDA route. Web mode `GPU (CUDA paged)` sends `runtime_backend=gpu`, desktop mode `GPU (CUDA paged)` routes through `run_local_deepseek_paged_decode_loop`, both surfaces report measured s/token plus pager/device telemetry, and the CPU exact MTP default remains unchanged at proof `state/mtp-warm-prefill-mtphead-evict-before-verify-rank-onepass-top2048-depth10-full61-tenvisible.json` (`34.2292s/visible-token`). Local proof is `state/deepseek-gpu-paged-ui-proof.json`; focused regression passes (`69 passed`).

## 2026-08-13 - Kimi, Gemma, Kronos, and product hardening

- Added generic Hugging Face, single-safetensor, PyTorch, and GGUF source inspection while preserving existing Qwen diagnostics.
- Added family-aware import validation and stale registry/download reconciliation.
- Added Kimi K2 and Gemma 3 GGUF chat-template routing through llama.cpp's OpenAI-compatible endpoint.
- Added the real CPU Kronos forecast adapter contract and `/api/forecast/kronos`; Kronos chat is guarded as unsupported.
- Restored the deleted web static files and replaced the hardcoded support list with the live compatibility matrix.
- Made the installed Qwen3-1.7B GGUF the live default when a compatible artifact is present.
- Added `state/model-compatibility-proof.json`; contract proof passes with `87 passed`, and the full suite passes with `560 passed, 2 skipped`.
- Completed desktop and 390px Playwright QA with zero browser console errors; the UI design-law audit passed.
