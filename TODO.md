...
- Before real monolithic production routing, add a C-side weight/session ABI that imports tensor residency pointers once per tensor and keeps them stable for the whole session. Without that, a monolithic wrapper would still bounce through Python loaders and would not remove the real crossing overhead.
