...
- Next DeepSeek runtime step: carry KV cache through FP8 MLA attention, add token embedding row loading, support dense layers `0-2`, and orchestrate all layers so the single-block proof becomes a real token decode path.
- Next DeepSeek speed step: replace materialized Python FP8 attention/expert dequant with fused/paged matmul so selected experts and attention weights do not become BF16 resident tensors.
