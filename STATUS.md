...
Phase Native Row8 Artifact Handles: native-owned row8 artifact handles landed; real Qwen 14B layer-0 and layers0-3 routes are correct (`Hello!`, `96/96`), full suite passes (`323 passed`), but artifact load lifecycle is still slower than baseline; next step is native mmap reuse.
