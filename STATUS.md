...
Phase Monolithic Forward scaffold: `pcketlm_forward.dll` now builds and exposes C-owned prefill/decode/verify/commit/rollback session semantics with `377` tests passing; real-model monolithic routing is not yet active because the C-side weight ownership ABI is still missing.
