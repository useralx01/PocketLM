...
Phase Native Row8 Artifact Handles: native-owned row8 artifact handles landed; real Qwen 14B layer-0 handle route is correct (`Hello!`, `96/96`), full suite passes (`323 passed`), but it is not faster than baseline until a multi-layer/whole-model row8 artifact exists.
