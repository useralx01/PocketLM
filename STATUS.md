...
Stage:
- multi-PC private prerelease preparation and compatibility verification

Current goal:
- publish a reproducible Windows test build with local installation supervision and GitHub CI proof
...
Current summary:
- The verified model-free archive is ready for private prerelease `v0.1.0-multi-pc.1`; it is 23.7 MB and excludes model weights, generated state, local configuration, and browser artifacts.
...
- Current DeepSeek V3 exact CPU default remains the shared-head warm-evict MTP proof at `34.2292s/visible-token`: `state/mtp-warm-prefill-mtphead-evict-before-verify-rank-onepass-top2048-depth10-full61-tenvisible.json`.
- Latest under-20 attempts did not clear a gate. The twenty-token sky proof accepted only `1/20` candidates and measured `617.3119s/visible-token`; q/kv attention role caching preserved exact output but regressed to `35.8912s/visible-token`; one-worker pack preload, small role-cache, and AVX-512 kill-switch probes were rejected or invalid. No under-20 artifact exists yet.
- Latest long-rank acceptance attempt is rejected. The opt-in `punctuation-rank-long-top2048` branch strategy was added for diagnostics and narrowed to request only the rank window it needs, but reduced probes wrote no JSON before the useful window expired. It is not a default and does not clear the under-20 phase.
- Latest exact full telemetry proof `state/mtp-preload-telemetry-full61-tenvisible.json` is ready/exact but diagnostic-only at `36.802s/visible-token`; it shows the real full verifier split as attention `105.0389s`, FFN `185.2804s`, router `21.673s`, expert preload `59.2928s`, routed expert math `81.1853s`, shared experts `11.5354s`, and dense FFN `7.3323s`.
- Router-only regular tensor caching was rejected despite a reduced win: full proof `state/mtp-routercache-full61-tenvisible.json` is ready/exact but regressed to `38.206s/visible-token`. The exact default remains `34.2292s/visible-token`.
...
- the current codebase has live compatibility profiles for Qwen, Qwen MoE, Mixtral, DeepSeek V3, Kimi K2, Gemma 3, and Kronos; implementation support and local-weight readiness are reported separately
...
- the restored Load Model screen shows the live catalog, reconciled download state, active runtime, GGUF lifecycle, and evidence-backed Proven/Experimental/Missing/Unsupported compatibility matrix

## 2026-08-13 - Storage-light multi-model compatibility

- Kimi K2 and Gemma 3 now import through family-aware validation and route GGUF chat through llama.cpp's native `/v1/chat/completions` template endpoint.
- Kronos now has a CPU-only forecast adapter at `/api/forecast/kronos`; chat requests are rejected as the wrong capability instead of being routed to an LLM backend.
- The registry catalog no longer repeats stale `runnable=true` when the live source/artifact is gone, and completed download records are relabeled `missing` when their target path is disconnected.
- The installed Qwen3-1.7B GGUF is the default live path. Kimi K2, Gemma 3, and Kronos remain `Missing` locally because their production weights were intentionally not downloaded.
- Proof: `state/model-compatibility-proof.json` reports `overall_pass=true` with `87 passed`; the full repository suite reports `560 passed, 2 skipped`.
...
PLM-13 Kaggle model source probe: official Kaggle DeepSeek V3 model source mounts and catalogs correctly, but one local pager layer plus streamed lm_head tail took `29.020s` on T4, so Kaggle input storage is not a viable `<=2s/token` runtime path.
PLM-13 exact CPU phase: meaningful lossless speed phase complete; repeated 8-layer first visible token is `0.943s`, cached 8-layer next-token row is `37.325s`, and full `<=10s/token` remains blocked by measured tail/attention/FFN compute.
...
PLM-13 exact CPU useful-answer fix: DeepSeek normal chat now uses 61 config layers and stops at EOS; corrected proof returns `Hello!!` with anti-cheat `183/183`, but speed is `277.921s/token`, so useful exact local `<=10s/token` remains blocked by CPU compute.
PLM-13 exact CPU native slice: weighted native routed MoE is exact and `1.181x` faster for real layer-3 MoE, native cached lm_head top-k is default, but 8-layer evidence still shows attention/FFN dominate and exact local `<=10s/token` remains blocked.
PLM-13 exact CPU full row: after the native matrix slice, full 61-layer exact token is `270.341s`; attention `127.197s`, FFN `70.877s`, lm_head `43.655s`, so `<=10s/token` is not reachable on this CPU path without a fundamentally faster matrix engine/hardware.
PLM-13 exact CPU follow-through: lm_head prefetch cuts full 61-layer exact token to `247.798s` and tail to `0.994s`; AVX2 FP8 gather, MTP candidates, and attention prefetch were tested and rejected by real DeepSeek evidence.
...
PLM-17 S1 update: internal NVMe bandwidth is measured at `1.6628 GiB/s` from a real 16 GiB FP8 pack probe (`state/plm-17-nvme-bandwidth-and-capacity.json`), versus `0.1015 GiB/s` on the current external Intenso USB path. Full source+pack migration is capacity-blocked: C: has `999.3 GB` total and about `555.5 GB` free, while DeepSeek source+pack requires `1,377,178,319,318` bytes; even the source or pack alone exceeds current C: free space.
PLM-18 S2 proof: residency split telemetry landed with focused FP8 tests passing (`34 passed`). Real DeepSeek bounded layers `0-7` proof is `state/plm-18-residency-split-layers8-maxnew2.json`: dense raw MLP spans resident (`1,189,375,488` bytes), shared raw spans resident (`220,254,720` bytes), attention dequantized cache resident for all 8 layers (`2,993,684,480` bytes), routed experts streaming-only (`0` routed span-cache bytes), `0` scattered reads, and `54.0303s/generated-token`.
PLM-19 S3 proof: AVX-512 FP8 weighted many-expert kernel landed as `fp8_linear_avx512.dll` and is now preferred when AVX-512F+BW is available. Focused native/runtime tests pass (`5 passed`, `35 passed`). Real proof `state/plm-19-avx512-fp8-moe.json` is bit-exact on the synthetic tiny oracle, real DeepSeek layer-3 full-size oracle, and bounded layers `0-7` forward output; bounded FFN timing improved from `11.8807s` to `11.2604s` (`1.0551x`) with attention/FFN per-layer timings recorded.
PLM-20 S4 proof: exact self-speculation uses only DeepSeek V3's MTP head and verifies each candidate batch in one full 61-layer weight sweep. Proof `state/plm-20-mtp-batched-full61-tenvisible-k8.json` generated `Blue, vast, endless, clouds, stars,` with `10` accepted visible tokens, `8` verifier sweeps, accepted-per-sweep `[1,1,1,1,1,1,2,2]` (`1.25` average), `549/549` anti-cheat layers, no blockers, and `569.1125s/visible-token`.
PLM-21 S5 final: the exact MTP batched verifier is now the default full-quality DeepSeek path for web and desktop, and both UIs surface live seconds/token via their runtime details. Final proof `state/plm-21-final-exact-default-full61-tenvisible-k8.json` is ready with strategy `deepseek-fp8-mtp-batched-exact`, generated `Blue, vast, endless, clouds, stars,`, `10` accepted visible tokens, `8` verifier sweeps, accepted-per-sweep `[1,1,1,1,1,1,2,2]`, `549/549` anti-cheat layers, no blockers, and `273.6789s/visible-token`. Focused regression suite passes: `115 passed`.
MTP tree speed phase final: the default exact DeepSeek path now uses a punctuation-biased MTP tree (`tree_width=8`, `tree_depth=2`, `max_tree_nodes=8`) so one full verifier sweep can commit word+punctuation pairs when full V3 agrees. Rank diagnosis `state/mtp-tree-diagnosis-plm21.json` showed the verifier token was in MTP top-k for `40/50` compared steps. Broad width-8/depth-3 was rejected as too slow and logged in `ERRORS.md`. Final default proof `state/mtp-tree-default-final-full61-tenvisible.json` generated the same `Blue, vast, endless, clouds, stars,` with `10` accepted tokens, `5` verifier sweeps, accepted-per-sweep `[2,2,2,2,2]`, `366/366` anti-cheat layers, no blockers, and `164.9231s/visible-token`. Regression slice passes: `116 passed`.
MTP adaptive depth-4 final: the default exact DeepSeek path now uses `tree_depth=4` with `branch_strategy=punctuation-next2`, which selectively tries the top-2 token after punctuation and commits `word, nextword,` chunks only when the full verifier agrees. Depth-3 and depth-5 variants were measured and rejected in `ERRORS.md`. Final default proof `state/mtp-adaptive-default-depth4-full61-tenvisible.json` generated `Blue, vast, endless, clouds, stars,` with `10` accepted tokens, `3` verifier sweeps, accepted-per-sweep `[4,4,2]`, `244/244` anti-cheat layers, no blockers, and `157.8086s/visible-token`. Regression slice passes: `116 passed`.
MTP under-100 exact default: the default exact DeepSeek path now warms the exact 61-layer prompt prefill cache, then uses adaptive finish depth so the final small budget can commit the remaining six visible tokens in one verifier sweep. Final proof `state/mtp-warm-prefill-finish6-depth4-full61-tenvisible.json` generated `Blue, vast, endless, clouds, stars,` with `10` accepted tokens, `2` verifier sweeps, accepted-per-sweep `[4,6]`, `183/183` anti-cheat layers, no blockers, `prefill_cache_hit=true`, warmup anti-cheat `61/61`, and `98.1757s/visible-token`. Web and desktop exact routes now surface the warmup proof plus measured MTP seconds/token. Regression slice passes: `119 passed`.
MTP around-50 attempt: exact-only follow-up tested broader finish depth, narrower continuation branch policies, and warm rank-pattern chains without changing the model. The top-k64 forced-sequence diagnostic `state/mtp-sequence-ranks-top64-sky11.json` proves the single-sweep 10-token route is blocked because the true 7th, 9th, and 11th tokens are missing from MTP top-k64. Rejected ready/stopped routes include finish-16 stopped at `46.9` minutes, forced-next2 finish stopped at `31.8` minutes, continuation-top1 at `113.1133s/visible-token`, warm rank-pattern depth-10 at `130.6426s/visible-token`, rank-first-next2 at `111.7026s/visible-token`, and unsafe MTP-head warmup with no JSON artifact. The promoted exact default is now warm-prefill `tree_depth=6` with `punctuation-rank-first-continuation-rank3`: proof `state/mtp-warm-prefill-rank-first-continuation-rank3-depth6-full61-tenvisible.json` generated `Blue, vast, endless, clouds, stars,` with accepted-per-sweep `[6,4]`, candidate tokens `[6,4]`, `183/183` anti-cheat layers, warmup anti-cheat `61/61`, no blockers, and `60.614s/visible-token`. Regression slice passes: `125 passed`.
MTP one-pass top-2048 final: exact-rank diagnostics showed the single-sweep route was blocked only by the old top-k64 cap, not by the MTP head itself. `state/mtp-sequence-exact-ranks-sky11.json` measured exact target ranks `[2,6,3,6,11,5,263,1,1304,2,808]`, and `state/mtp-sequence-ranks-top2048-sky11.json` confirmed actual top-k positions `[1,6,2,5,11,5,262,1,1303,2,808]`. The promoted default is now `tree_depth=10`, `mtp_top_k=2048`, `branch_strategy=punctuation-rank-onepass-top2048`. Final proof `state/mtp-warm-prefill-rank-onepass-top2048-depth10-full61-tenvisible.json` generated `Blue, vast, endless, clouds, stars,` with accepted-per-sweep `[10]`, candidate tokens `[10]`, one verifier sweep, `122/122` measured anti-cheat layers, warmup anti-cheat `61/61`, no blockers, and `37.4887s/visible-token`. Regression slice passes: `127 passed`.
MTP around-20 attempt: no faster exact default was promoted. Depth-20, fixed-rank 20-token continuation, root-tail skip, larger caches, direct MTP shared-head warmup, and prefill attention prefetch were all measured and rejected in `ERRORS.md`; each preserved the hard constraints but either failed readiness or measured slower than `37.4887s/visible-token`. The current exact default remains `state/mtp-warm-prefill-rank-onepass-top2048-depth10-full61-tenvisible.json` with `tree_depth=10`, `mtp_top_k=2048`, one verifier sweep, and output `Blue, vast, endless, clouds, stars,`. Regression slice passes: `127 passed`.
MTP shared-head warm-evict default: the default exact DeepSeek path now warms V3's own MTP shared head during exact prompt warmup, then evicts only `model.layers.61.shared_head.head.weight` before the full verifier sweep. Proof `state/mtp-warm-prefill-mtphead-evict-before-verify-rank-onepass-top2048-depth10-full61-tenvisible.json` generated `Blue, vast, endless, clouds, stars,` with accepted-per-sweep `[10]`, candidate tokens `[10]`, one verifier sweep, `122/122` measured anti-cheat layers, warmup anti-cheat `61/61`, no blockers, MTP tail sum `7.1937s`, verifier loop `282.188s`, and `34.2292s/visible-token`. Web runtime details now surface MTP shared-head warmup, targeted eviction, and verifier timing fields. Regression slice passes: `128 passed`.
MTP regular tensor cache attempt rejected: implemented an opt-in exact bounded process cache for catalog regular tensors plus router timing telemetry and UI runtime exposure, but proof `state/mtp-regular-cache-mtphead-evict-rank-onepass-top2048-depth10-full61-tenvisible.json` regressed to `42.6497s/visible-token` despite exact output and `122/122` anti-cheat. The cache is not enabled by the exact default. Regression slice passes: `129 passed`.
MTP row-weighted many probe rejected: added an opt-in verifier-shape native weighted-many MoE probe, but reduced proof `state/mtp-row-weighted-many-enabled-layers8-visible10.json` did not improve verifier FFN time (`13.995s` vs baseline `13.5641s`). No full proof or default promotion.
MTP thread tuning probe rejected: `PCKETLM_FP8_CPU_THREADS=14` regressed the reduced verifier loop to `31.3808s` versus the default-thread baseline `21.6892s`; no default change.
MTP streamed attention probe rejected: `PCKETLM_ENABLE_STREAMED_FP8_ATTENTION=1` regressed the reduced verifier loop to `28.4998s` versus baseline `21.6892s`; no default change.
MTP no-MLP-span cache probe rejected: reduced layers looked faster, but full proof `state/mtp-no-mlpspan-cache-mtphead-evict-rank-onepass-top2048-depth10-full61-tenvisible.json` regressed to `47.7969s/visible-token` despite exact output and `122/122` anti-cheat; no default change.
MTP attention FP32 cast-cache probe rejected: opt-in exact FP32 casts of BF16 attention weights regressed the reduced verifier loop to `51.2504s`; no full proof or default change.
MTP top-k1304 probe rejected: full proof stayed exact but regressed to `39.6258s/visible-token`; keep `mtp_top_k=2048`.
MTP MoE worker probe rejected: `PCKETLM_FP8_MOE_EXPERT_WORKERS=4` regressed the reduced verifier loop to `24.3151s`; no default change.
MTP attention matmul-core probe rejected: reduced proof improved, but full proof accepted only `6/10` candidates and was not ready, so the einsum attention core remains default.
MTP below-20 work state: default remains `34.2292s/visible-token`; latest regression slice passes: `132 passed`.
MTP native many-MLP kill-switch probe rejected: disabling native many-MLP regressed the reduced verifier loop to `23.7968s`; keep it enabled.
MTP AVX-512 full-MLP probe rejected: a bit-exact opt-in AVX-512 full-MLP kernel landed and passes the focused slice (`133 passed`), but reduced proof `state/mtp-avx512-fullmlp-raw-layers8-visible10.json` regressed to `36.8142s/visible-token` versus the `33.6825s` reduced baseline, so the exact default remains `34.2292s/visible-token`.
MTP row-weighted native-batch MoE probe rejected: new opt-in row-weighted many-expert kernel passes the focused slice (`134 passed`) and reduced proof improved only to `32.9885s/visible-token`; the full proof overran the under-20 wall-time gate and was stopped, so no default change.
MTP attention-side probes rejected: narrow `kv_b` FP32 cast (`43.7692s` reduced), q/kv role cache plus row-weighted MoE (`33.0016s` reduced), native attention-linear (`55.5384s` reduced), and causal-mask caching (`36.0663s` reduced, reverted) did not produce a phase path. Focused slice remains `134 passed`.
MTP longer-branch/cache follow-up rejected: repeated/counting top-k64 reduced rank probes missed too many targets, disabling native full-MLP regressed to `39.3835s` reduced, and q/kv cache budget/policy variants did not beat the known q/kv/full rejection. Default remains `34.2292s/visible-token`.
MTP SDPA attention-core probe rejected and removed: reduced proof `state/mtp-attn-sdpa-core-layers8-visible10.json` regressed to `43.8172s/visible-token`; focused slice remains `134 passed`.
MTP native FP8 batch-reuse probe rejected: opt-in batch reuse for FP8 linear/dual/full-MLP plus row-weighted routed MoE is bit-exact in focused tests (`62 passed`) and improved the reduced verifier-only depth-10 loop from `28.7857s` to `24.2393s`, but the full 61-layer proof with `PCKETLM_ENABLE_NATIVE_FP8_BATCH_REUSE=1` and row-weighted MoE produced no ready JSON after `867.6s` wall / `3642.5469` CPU seconds and was stopped. Snapshot: `state/mtp-batch-reuse-rowweighted-mtphead-evict-rank-onepass-top2048-depth10-full61-tenvisible-stopped.json`. No default promotion; current exact default remains `34.2292s/visible-token`.
MTP u16 projection probes rejected: native u16 and Torch BF16 attention projection modes changed the reduced verifier token stream and regressed verifier-only timing (`60.4135s` and `45.347s` loops versus the `28.7857s` exact baseline). Keep `PCKETLM_ENABLE_NATIVE_U16_WEIGHT_LINEAR` and `PCKETLM_ENABLE_TORCH_U16_WEIGHT_LINEAR` off for the exact MTP verifier.
MTP MoE worker-2 probe rejected: `state/mtp-moe-workers2-layers8-visible10.json` measured `34.2567s/visible-token` on the reduced 8-layer proof, slower than the comparable reduced baseline; no default change.
MTP native batch-reuse-only probe rejected: `state/mtp-batchreuse-only-layers8-visible10.json` with `PCKETLM_ENABLE_NATIVE_FP8_BATCH_REUSE=1` measured `38.8915s/visible-token` on the reduced proof, with FFN rising to `19.0801s`; keep batch reuse opt-in and unpromoted.
MTP AVX-512 LUT-gather full-MLP probe rejected: the exact native tests passed (`10 passed`) and reduced proof `state/mtp-avx512-lut-fullmlp-layers8-visible10.json` measured `35.1863s/visible-token` versus the scalar reduced baseline around `33.6825s`. The LUT-gather source change was reverted so the default weighted-MoE draft path stays on the prior bit-decode helper.
MTP q/kv plus pack-worker probe rejected: `state/mtp-qkv-packworkers16-layers8-visible10.json` measured `32.5517s/visible-token` on the reduced proof, worse than the q/kv-only reduced proof and not a full-proof candidate.
MTP pack-worker32 probe rejected: `state/mtp-packworkers32-layers8-visible10.json` measured `34.925s/visible-token` reduced, slower than worker `16`; no default change.
MTP long-rank window reduced probe rejected: `state/mtp-longrank-window-layers8-visible10.json` accepted only one reduced token and measured `36.4668s/visible-token`; no full proof.
MTP pack-worker16 full proof rejected: `state/mtp-packworkers16-full61-tenvisible.json` produced no JSON after a 15-minute timeout and the leftover benchmark process was stopped; no default change.
MTP native thread-4 probe rejected: `state/mtp-native-threads4-layers8-visible10.json` measured `36.6971s/visible-token` reduced; keep `PCKETLM_NATIVE_THREADS` unset.
MTP norm-only regular cache probe rejected: `state/mtp-normcache-layers8-visible10.json` cached only `352256` bytes but regressed to `36.1806s/visible-token` reduced; keep regular cache disabled.
MTP below-20 verification: after reverting the rejected AVX-512 LUT-gather experiment and logging the latest cache/worker/thread probes, native DLL rebuild plus focused regression slice pass (`137 passed`). Current promoted exact default remains `34.2292s/visible-token`; no `20s` phase proof exists.
MTP inference-mode one-pass probe rejected: wrapping the current exact MTP path in `torch.inference_mode()` produced no reduced layers-8 JSON before the 3-minute stop, so no proof artifact exists and the default remains `34.2292s/visible-token`.
MTP terminal verifier trim rejected: opt-in trim proof `state/mtp-terminal-trim-mtphead-evict-rank-onepass-top2048-depth10-full61-tenvisible.json` stayed exact but regressed to `41.4318s/visible-token`; the trim is now gated behind `PCKETLM_ENABLE_FP8_MTP_TERMINAL_VERIFIER_TRIM` and the promoted default remains `34.2292s/visible-token`.
MTP hot-cache mmap probe rejected for default: `PCKETLM_ENABLE_FP8_HOT_CACHE_MMAP=1` improved the reduced non-ready layers-8 proof to `28.8082s/visible-token`, but the full 61-layer proof wrote no JSON before a 20-minute stop and was killed; no default change and no under-20 proof.
MTP current-default rerun rejected: `state/mtp-current-default-rerun-full61-tenvisible.json` is exact/ready but slower at `53.0351s/visible-token`; disabling FP8 hot cache then produced no reduced JSON before the stop. The promoted proof remains `34.2292s/visible-token`; no `20s` phase proof exists.
MTP attention dequant copy-read probe rejected: `PCKETLM_DISABLE_FP8_DEQUANT_HOT_CACHE_MMAP=1` preserved the reduced exact prefix but regressed to `49.8599s/visible-token`; keep attention dequant hot-cache mmap enabled and unmodified for the exact default.
MTP CPU thread-8 probe rejected: `PCKETLM_FP8_CPU_THREADS=8` preserved reduced exactness but regressed to `55.6239s/visible-token`; keep FP8 CPU thread tuning unset.
MTP correctly scoped inference-mode probe rejected: fresh reduced baseline is `33.9547s/visible-token`, while `state/mtp-infermode-correct-layers8-visible10.json` regressed to `41.2255s/visible-token`; no default change.
MTP forced-root continuation rejected and reverted: `state/mtp-onepass-windowed-topk-layers8-visible10.json` built 10 candidates after a reduced root miss, accepted only 1, and regressed to `89.2289s/visible-token`; root misses again force only the first exact token.
MTP one-pass windowed top-k rejected for default: `state/mtp-onepass-windowed-topk-safe-layers8-visible10.json` preserved the reduced prefix but regressed to `36.6361s/visible-token` versus the fresh `33.9547s` baseline; it is gated behind `PCKETLM_ENABLE_FP8_MTP_ONEPASS_WINDOWED_TOPK=1`.
MTP pack-worker16 plus row-weighted MoE rejected: `state/mtp-packworkers16-rowweighted-layers8-visible10.json` preserved reduced exactness but regressed to `36.32s/visible-token`; no default change.
MTP-layer attention pin rejected: opt-in `PCKETLM_FP8_ATTENTION_WEIGHT_CACHE_PIN=model.layers.61.self_attn` full proof `state/mtp-mtplayer-attnpin-full61-tenvisible.json` stayed exact but regressed to `38.2928s/visible-token`; keep the default prefix attention cache unpinned.
MTP-layer attention pin with larger cache rejected: `state/mtp-mtplayer-attnpin-cache4608-full61-tenvisible.json` stayed exact but regressed to `53.9244s/visible-token`; do not raise attention cache size for the exact default.
MTP shared/preload overlap rejected: `state/mtp-overlap-shared-preload-layers8-visible10.json` preserved reduced exactness but regressed to `49.319s/visible-token`; keep `PCKETLM_ENABLE_FP8_MOE_OVERLAP_SHARED_PRELOAD` off by default.
MTP verifier-scope MoE follow-up rejected: verifier-only batchreuse+rowweighted and rowweighted-only full proofs both overran memory before writing ready JSONs at about `24.0 GB` private memory, and the active-pair MoE kernel only produced a reduced branch verifier `34.7212s` versus rowweighted `33.7731s` and default `35.802s`. Current promoted exact default remains `34.2292s/visible-token`; no `20s` phase proof exists.
MTP below-20 verifier follow-up: focused regression slice passes (`147 passed`), and additional reduced verifier gates were rejected with artifacts: low-cache batchreuse+rowweighted (`32.5597s` verifier), batchreuse+active-pair (`30.0551s`), trimshape9+batchreuse+rowweighted (`32.6593s`), verifier attention matmul-core (`46.0656s`), and `OMP_NUM_THREADS=4` on the combined 4-layer branch (`13.0229s` vs `8.0943s`). Current promoted exact default remains `34.2292s/visible-token`; no `20s` phase proof exists.
MTP below-20 cache follow-up: opt-in `PCKETLM_FP8_MLP_SPAN_CACHE_POLICY=lru` now allows routed-cache diagnostics without changing defaults, and the focused regression slice passes (`148 passed`). Reduced warm probes found tempting cache signals, but full exact proofs did not yield a stable default: routed-all 2GB measured `36.5052s/visible-token`, shared-only 3GB measured `36.1748s`, manual shared-only 2GB measured `33.7949s`, and the patched-default rerun for shared-only 2GB regressed to `35.4807s`. Current promoted exact default remains `34.2292s/visible-token`; no `20s` phase proof exists.
MTP below-20 row-weighted chunk follow-up: opt-in `PCKETLM_FP8_MOE_ROW_WEIGHTED_MANY_CHUNK` landed for memory-bounded verifier diagnostics and the focused runtime/native slice passes (`74 passed`), but reduced fixed-branch proofs regressed (`42.029s` for chunk8 and `41.5318s` for chunk32 versus `35.802s` default and `26.5001s` unchunked diagnostic). Current promoted exact default remains `34.2292s/visible-token`; no `20s` phase proof exists.
MTP canonical sky-20 branch rejected: the full verifier does not provide twenty useful visible tokens on the current sky prompt. `state/mtp-warm-prefill-rank-onepass20-top2048-depth20-full61-visible20.json` accepted only `11/20` candidates and measured `61.056s/visible-token`; the forced rank diagnostic continues past the verifier EOS/comma decision into the wrong `sun. sun,1,1,1<EOS>` branch. Latest focused regression slice passes (`149 passed`), and the exact default remains `34.2292s/visible-token`; no `20s` phase proof exists.
MTP long-branch scaling rejected for the under-20 gate: added `tools/bench_fp8_mtp_verifier_scaling.py` and a diagnostic-only fixed-rank MTP selector for prompt-specific rank probes. Reduced 8-layer scaling improved to `2.0399s/candidate` at length 40, but full 61-layer scaling `state/mtp-verifier-scaling-skybranch-full61-len40.json` measured `1289.7452s` for 40 verifier positions (`32.2436s/candidate`) before MTP proposal cost. The color-list prompt accepted only `2/40`, and repeat-blue top-1 overran the phase window before writing a ready proof. Current exact default remains `34.2292s/visible-token`; no `20s` phase proof exists.
MTP native attention sequence probe rejected: opt-in `PCKETLM_ENABLE_NATIVE_FP8_ATTENTION_LINEAR_SEQ=1` was fixed after a shape bug, but reduced proof `state/mtp-attn-native-linear-seq-layers8-visible10.json` changed the verifier stream (`0/10` accepted) and regressed attention time to `19.5593s` versus the clean 8-layer branch at `6.2404s`. Current exact default remains `34.2292s/visible-token`; no `20s` phase proof exists.
DeepSeek GPU path added: web and desktop now expose opt-in `GPU (CUDA paged)` mode backed by the local DeepSeek FP8 resident-layer pager, with runtime telemetry in `fp8_runtime.gpu_paged` / `gpu_paged` and measured GPU s/token fields when CUDA is present. Local proof `state/deepseek-gpu-paged-ui-proof.json` records catalog/pack ready, projected resident hot path `1.7642177491108302s/token`, and the local blocker `CUDA is not available on this machine.` Focused regression passes: `69 passed`.
