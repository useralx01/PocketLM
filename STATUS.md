...
Phase FP8 Lossless Pack: full DeepSeek FP8 pack exists and passes byte-identity/output-equivalence/62-layer anti-cheat, but the timing gate is partial because grouped pack reads are only `10.52%` faster than scattered (`741.916s` vs `829.127s`), not the required `50%`.
